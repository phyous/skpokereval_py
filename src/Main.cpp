// FROM: https://github.com/gsugar87/SKPokerPython
#include <Python.h>
#include <iostream>
#include <cstdio>
#include <ctime>
...
static PyObject* getRank(PyObject* self, PyObject *args){
    PyObject * ourCardsObj;
    PyObject * boardCardsObj;
    PyObject * floatObj;

    if (!PyArg_ParseTuple(args, "OO", &ourCardsObj, &boardCardsObj)) return NULL;

    int numBoardCards = PyList_GET_SIZE(boardCardsObj);
    ...
    for (int i=0; i<2; i++){
        floatObj = PyList_GetItem(ourCardsObj, i);
    ...
    for (int i=0; i<numBoardCards; i++){
        floatObj = PyList_GetItem(boardCardsObj, i);
        boardCards.push_back(PyLong_AsLong(floatObj));
    }

    int ret = getInitialRank(ourCards, boardCards);
...
static PyMethodDef HandEvaluatorCPPMethods[] = {
        ...
        {"getRank", getRank, METH_VARARGS, "Get the rank of a texas hold'em hand."},
        {NULL, NULL, 0, NULL}
};

static struct PyModuleDef inithandEvaluatorCPP =
{
    PyModuleDef_HEAD_INIT,
    "handEvaluatorCPP", /* name of module */
    "Evaulate the strength of a texas hold'em hand.",          /* module documentation, may be NULL */
    -1,          /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    HandEvaluatorCPPMethods
};

PyMODINIT_FUNC PyInit_handEvaluatorCPP(void) {
    Py_Initialize();
    return PyModule_Create(&inithandEvaluatorCPP);
}